...
static double GetSpan(std::chrono::system_clock::time_point time1, std::chrono::system_clock::time_point time2) 
{
        ...
        return 1000 * double(duration.count()) * std::chrono::microseconds::period::num / std::chrono::microseconds::period::den;
}
...
static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    cv::imwrite("yolov5s-tt100k.out.jpg", image);
}
...
int main(int argc, char* argv[])
{
    ...
    auto draw_objects_time = std::chrono::system_clock::now();
    draw_objects(img, objects);
    printf("draw_objects_time %f ms.\n", GetSpan(draw_objects_time, std::chrono::system_clock::now()));
